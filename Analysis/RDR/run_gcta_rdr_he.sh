...
if [[ -z "$1" || -z "$2" ]]; then
    echo "Usage: ./run_GCTA_rdr_HEreg.sh <path_to_run_folder_on_scratch> <path_to_final_results_dir_on_projects>"
    exit 1
fi
...
TARGET_SAMPLE_SIZES=(32000)

echo "--- Starting RDR Haseman-Elston regression analysis for ${RUN_ID} using custom GRM calculation ---"
...
for N in "${TARGET_SAMPLE_SIZES[@]}"; do
    ...
    WORK_DIR="${RUN_FOLDER}/gcta_hereg_analysis/N_${N}"
    OUTPUT_PREFIX="${FINAL_RESULTS_DIR}/${RUN_ID}_HEreg_N${N}"
    ...
    # Step 3: Run Univariate Haseman-Elston regression with the correctly calculated GRMs
    echo "Step 3: Running RDR Haseman-Elston regression for Trait 1 (Y1) with N=${N}..."
    echo "Step 3a (Y1) started at: $(date)"
    STEP3A_START_TIME=$(date +%s)
    
    gcta64 --HEreg --mgrm "${WORK_DIR}/mgrm.txt" \
           --pheno "${WORK_DIR}/offspring.phen" --mpheno 1 \
           --out "${OUTPUT_PREFIX}_Y1" \
           --thread-num 8 || echo "WARNING: GCTA HEreg for Y1 failed at N=${N}."
    ...
    gcta64 --HEreg --mgrm "${WORK_DIR}/mgrm.txt" \
           ...
           --thread-num 8 || echo "WARNING: GCTA HEreg for Y2 failed at N=${N}."
    ...
    echo "--- Finished Haseman-Elston analysis for N=${N} at: $(date) (Total duration: ${N_DURATION}s) ---"
    echo "Timing breakdown for N=${N}:"
    echo "  - Step 1 (GRM calculation): ${STEP1_DURATION}s"
    echo "  - Step 2 (Multi-GRM file): ${STEP2_DURATION}s"
    echo "  - Step 3a (Y1 HEreg): ${STEP3A_DURATION}s"
    echo "  - Step 3b (Y2 HEreg): ${STEP3B_DURATION}s"
