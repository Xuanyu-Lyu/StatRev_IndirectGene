#!/bin/bash
# SLURM job submission script for RDR regression analysis
...
for CONDITION_NAME in "${CONDITIONS[@]}"; do
    ...
    FINAL_RESULTS_DIR_ML="/projects/xuly4739/Py_Projects/StatRev_IndirectGene/Analysis/RDR_Results_Check/${CONDITION_NAME}_ML"
    FINAL_RESULTS_DIR_HE="/projects/xuly4739/Py_Projects/StatRev_IndirectGene/Analysis/RDR_Results_Check/${CONDITION_NAME}_HE"
    ...
    ./run_gcta_rdr_v2.sh "${RUN_FOLDER}" "${FINAL_RESULTS_DIR_ML}"
    ./run_gcta_rdr_he.sh "${RUN_FOLDER}" "${FINAL_RESULTS_DIR_HE}"
