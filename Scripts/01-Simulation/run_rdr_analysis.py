...
def main():
    ...
    # --- 2. Generate Task List (remains the same) ---
    tasks = []
    ...
    print(f"Total analysis tasks to run: {len(tasks)}")

    # --- 3. Run All Tasks Sequentially in a For Loop (MODIFIED) ---
    ...
    for i, task in enumerate(tasks):
        ...
        # Print the estimates to the log file as they are generated
        if result.get('status') == 'success':
            print("  RDR Estimates:")
            print(f"    v_g: {result.get('v_g_hat'):.4f} (SE: {result.get('se_v_g'):.4f})")
            print(f"    v_e_g: {result.get('v_e_g_hat'):.4f} (SE: {result.get('se_v_e_g'):.4f})")
            print(f"    c_g_e: {result.get('c_g_e_hat'):.4f} (SE: {result.get('se_c_g_e'):.4f})")
            print(f"    sigma^2: {result.get('sigma2_hat'):.4f} (SE: {result.get('se_sigma2'):.4f})")
        else:
            print(f"  Task failed or errored: {result.get('error', 'Unknown error')}")
        print("-" * 50)


    # --- 4. Aggregate and Save Results (remains the same) ---
